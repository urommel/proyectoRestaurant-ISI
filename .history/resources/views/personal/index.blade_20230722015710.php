...
@section('content')
    <div class="container-fluid">
        <!-- Agrega la alerta con el nombre del día aquí -->
        <div class="alert alert-info m-0" role="alert">
            Hoy es {{ ucfirst($diaActual) }}
        </div>
        <div class="row">
            <div class="col-sm-12">
                <div class="card">
                                ...
                                <tbody>
                                    @foreach ($personals as $personal)
                                        <tr>
                                            ...
                                            <td class="text-center">
                                                @if (!isset($asistencias) || $asistencias->isEmpty() || !isset($asistencias->first()->personal_id))
                                                    <!-- Si no hay asistencias asignadas o no se ha asignado el día actual, muestra el botón de asignar -->
                                                    <a class="btn btn-sm btn-success m-1"
                                                        href="{{ route('asistencia.create', ['id' => $personal->id]) }}"><i
                                                            class="fas fa-vote-yea p-1"></i></a>
                                                @else
                                                    <!-- Si hay asistencias asignadas -->
                                                    @php
                                                    $diaActual = $this->diasemanaactual();
                                                    $diasAsignados = $personal->horarios->pluck('dia')->map(fn ($dia) => strtolower($dia));
                                                @endphp

                                                @if (!$diasAsignados->contains(strtolower($diaActual)))
                                                    <!-- Si el día actual no está asignado en los horarios del personal, muestra el mensaje de advertencia -->
                                                    <div class="alert alert-warning m-0" role="alert">
                                                        No puedes asignar asistencia hoy.
                                                    </div>
                                                @elseif (!isset($asistencias) || $asistencias->isEmpty() || !isset($asistencias->first()->personal_id))
                                                    <!-- Si no hay asistencias asignadas o no se ha asignado el día actual, muestra el botón de asignar -->
                                                    <a class="btn btn-sm btn-success m-1"
                                                        href="{{ route('asistencia.create', ['id' => $personal->id]) }}"><i class="fas fa-vote-yea p-1"></i></a>
                                                @else
                                                    <!-- Si el día actual está asignado en los horarios del personal, muestra el botón de ver asistencias -->
                                                    <a class="btn btn-sm btn-success m-1"
                                                        href="{{ route('asistencia.show', $asistencias->first()->personal_id) }}"><i class="fas fa-th-list p-1"></i></a>
                                                @endif
                                                @endif
                                            </td>
