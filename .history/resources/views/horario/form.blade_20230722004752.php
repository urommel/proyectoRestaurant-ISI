                ...
                @foreach (['Lunes', 'Martes', 'Miércoles', 'Jueves', 'Viernes', 'Sábado', 'Domingo'] as $dia)
                    ...
                    <tr>
                        ...
                        <td>
                            <div class="checkbox-container">
                                <input type="checkbox" name="horarios[{{ strtolower($dia) }}][trabaja]" value="1"
                                    id="checkbox-{{ strtolower($dia) }}" class="trabaja-checkbox" {{ in_array(strtolower($dia), $horariosExistentes) ? 'checked' : '' }} disabled>
                                <label for="checkbox-{{ strtolower($dia) }}"></label>
                            </div>
                        </td>
                        <td>
                            <input type="time" name="horarios[{{ strtolower($dia) }}][hora_inicio]"
                                class="form-control hora-inicio {{ in_array(strtolower($dia), $horariosExistentes) ? 'horario-creado' : '' }}" disabled>
                        </td>
                        <td>
                            <input type="time" name="horarios[{{ strtolower($dia) }}][hora_fin]"
                                class="form-control hora-fin {{ in_array(strtolower($dia), $horariosExistentes) ? 'horario-creado' : '' }}" disabled>
                        </td>
                    </tr>
